...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if(pid.pgcateg() == BTreePageId.LEAF){
			...
			BTreeInternalPage currentPage = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
			BTreeEntry currentEntry;
			Iterator<BTreeEntry> entries = currentPage.iterator();
			if(entries.hasNext()){
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
        ...
        Stack<Tuple> tuplesToShift = new Stack<Tuple>();

        Iterator<Tuple> revIt = page.reverseIterator();
        // For 2n+1 tuples, an n:n+1 split is done and for 2n tuples, an n:n split is done.
        int totalTuples = page.getNumTuples();
        int firstKeyToMove = totalTuples/2;
        for(int i = firstKeyToMove; i < totalTuples ; i++){
            if(revIt.hasNext()){
                tuplesToShift.push(revIt.next());
            }
        }

        while(!tuplesToShift.empty())
        {
            Tuple t = tuplesToShift.pop();
            page.deleteTuple(t);
            newPage.insertTuple(t);
        }

        Field fieldToBeCopied = page.reverseIterator().next().getField(keyField);
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), fieldToBeCopied);
        BTreeEntry copyUpEntry = new BTreeEntry(fieldToBeCopied, page.getId(), newPage.getId());
        parent.insertEntry(copyUpEntry);
        page.setParentId(parent.getId());
        newPage.setParentId(parent.getId());

        if(page.getRightSiblingId() != null) {
            BTreeLeafPage oldSibling = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            oldSibling.setLeftSiblingId(newPage.getId());
        }
        newPage.setRightSiblingId(page.getRightSiblingId());
        newPage.setLeftSiblingId(page.getId());
        page.setRightSiblingId(newPage.getId());

        if(field.compare(Op.LESS_THAN_OR_EQ, fieldToBeCopied))
            return page;
        else
            return newPage;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
        ...
        Stack<BTreeEntry> entriesToShift = new Stack<BTreeEntry>();

        Iterator<BTreeEntry> revIt = page.reverseIterator();
        // For 2n+1 tuples, an n:n+1 split is done and for 2n tuples, an n:n split is done.
        int totalEntries = page.getNumEntries();
        int midEntryNum = totalEntries/2;

        for(int i = midEntryNum; i < totalEntries ; i++){
            if(revIt.hasNext()){
                entriesToShift.push(revIt.next());
            }
        }
        BTreeEntry pushUpEntry = entriesToShift.pop();
        page.deleteKeyAndRightChild(pushUpEntry);

        while(!entriesToShift.empty())
        {
            BTreeEntry e = entriesToShift.pop();
            page.deleteKeyAndRightChild(e);
            newPage.insertEntry(e);
        }

        // update the parentIds of the children of page and newPage
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		Iterator<Tuple> entries;
		if(isRightSibling){
			rightPage = sibling;
			entries = sibling.iterator();
		} else {
			rightPage = page;
			entries = sibling.reverseIterator();
		}
		int numOfTuplesNeed = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		Tuple[] tuples = new Tuple[numOfTuplesNeed];
		int count = 0;
		while(count < numOfTuplesNeed && entries.hasNext()){
			tuples[count] = entries.next();
			count++;
		}
		for (Tuple t : tuples) {
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numOfEntriesNeeded = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
		BTreeEntry[] entriesToShift = new BTreeEntry[numOfEntriesNeeded];

		Iterator<BTreeEntry> entries = leftSibling.reverseIterator();
		int count = 0;

		while (count < numOfEntriesNeeded && entries.hasNext()) {
			entriesToShift[count] = entries.next();
			count++;
		}

		for (int i=0; i < numOfEntriesNeeded; i++) {
			BTreeEntry entry = entriesToShift[i];
			// delete entry from left sibling
			leftSibling.deleteKeyAndRightChild(entry);
			updateParentPointer(tid, dirtypages, page.getId(), entry.getRightChild());
			BTreePageId rightChildOfEntry = entry.getRightChild();
			...
			parentEntry.setLeftChild(rightChildOfEntry);
			...
			// traverse through parent
			parentEntry = entry;
		}
		//update dirty pages with changed pages.
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numOfEntriesNeeded = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		BTreeEntry[] entriesToShift = new BTreeEntry[numOfEntriesNeeded];
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Tuple[] tuplesToMove = new Tuple[rightPage.getNumTuples()];
		int count = 0;
		while (count< rightPage.getNumTuples() && tuples.hasNext()) {
			tuplesToMove[count++] = tuples.next();
		}
		for (int i=0; i < tuplesToMove.length; i++) {
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> movableEntries = rightPage.iterator();
		while(movableEntries.hasNext()){
			BTreeEntry entry = movableEntries.next();
