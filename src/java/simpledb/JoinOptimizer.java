...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        int numOfJoins = joins.size();
        for (int i = 1; i <= numOfJoins; i++) {
            Set<Set<LogicalJoinNode>> allSubsets = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> innerSet : allSubsets) {
                double bestcost = Double.MAX_VALUE;
                CostCard bestPlan = null;
                for (LogicalJoinNode subPlan : innerSet) {
                    CostCard cost = computeCostAndCardOfSubplan(stats, filterSelectivities, subPlan, innerSet, bestcost, planCache);
                    if (cost != null) {
                        if (cost.cost < bestcost || bestPlan == null) {
                            bestcost = cost.cost;
                            bestPlan = cost;
                        }
                        planCache.addPlan(innerSet, bestcost, bestPlan.card, bestPlan.plan);
                    }
                }
            }
        }

        Set<LogicalJoinNode> finalSet = this.enumerateSubsets(this.joins, numOfJoins).iterator().next();
